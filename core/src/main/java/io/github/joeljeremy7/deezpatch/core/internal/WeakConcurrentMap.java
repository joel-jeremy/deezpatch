...
/** A simple weak concurrent map implementation which only holds weak references to the keys. */
@Internal
public class WeakConcurrentMap<K, V> {
   ...
   *     null} if the computed value is {@code null}.
   */
  public V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
    return map.computeIfAbsent(
        WeakKey.forWrite(key, referenceQueue), k -> mappingFunction.apply(k.get()));
  }
